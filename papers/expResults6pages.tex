...
\vspace{-1em}
\caption{\label{fig:Learning}(Left) The total number of games played per player drops off exponentially. (Right) We are able to show that players skill improves as they retry tests using data from \emph{Varying Number}
\vspace{-2em}
}
\end{figure}
...
\begin{figure*}
\subfloat[][Vary number]{\label{fig:timePlayed}
\begin{overpic}[width = .6\columnwidth]{timePlayed.pdf}\end{overpic}}
%
\subfloat[][Vary control]{\label{fig:ResVaryNu}
\begin{overpic}[width = .7\columnwidth]{ResVaryNum.pdf}\end{overpic}}
%
\subfloat[][Vary visualization]{\label{fig:ResVaryNoise}
\begin{overpic}[width = .7\columnwidth]{ResVaryNoise.pdf}\end{overpic}}
%
\caption{\label{fig:someexperimentresults}(a) Cumulative time played for completed tests.
(b) Data from \emph{Varying Number} using robots to push an object through a maze to a goal location.  The data indicates that this task has an optimal number of robots, perhaps due to the relative sizes of the robots, obstacles, and object. Best-fit linear and quadratic lines are overlaid for comparison. (c) Varying the noise from 0 to 200\% of the maximum control input resulted in only a small increase in completion time.
\vspace{-2em}
}
\end{figure*}
...
%\begin{figure}
%\centering
%\begin{overpic}[width = .7\columnwidth]{ResVaryControl.pdf}\end{overpic}
%\vspace{-1em}
...
\begin{figure*}
\subfloat[][Vary control]{\label{fig:ResVaryControl}
\begin{overpic}[width = .7\columnwidth]{ResVaryControl.pdf}\end{overpic}}
%
\subfloat[][Vary visualization]{\label{fig:ResVaryVis}
\begin{overpic}[width = .7\columnwidth]{ResVaryVis.pdf}\end{overpic}}
%
\subfloat[][Vary number]{\label{fig:ResPositioning}
\begin{overpic}[width = .6\columnwidth]{ResPositioning.pdf}\end{overpic}}
%
\caption{\label{fig:someexperimentresults}(a) Attractive control resulted in the shortest completion time and repulsive the longest for building a three-block pyramid. (b) Completion-time results for the four levels of visual feedback shown in Fig.~\ref{fig:Visualization}. Surprisingly, players perform better with limited feedback--subjects with only the mean + variance  outperformed all others. (c) Increasing the number of robots resulted in longer completion times.  For more than 4 robots the goal pattern contained a void, which may have caused the longer completion times.
\vspace{-2em}
}
\end{figure*}
